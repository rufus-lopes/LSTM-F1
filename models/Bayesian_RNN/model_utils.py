...
def variationalPosterior(shape, name, prior, istraining):
    ...
    mu=tf.compat.v1.get_variable("{}_mean".format(name), shape=shape, dtype=tf.float32);
    rho=tf.compat.v1.get_variable("{}_rho".format(name), shape=shape, dtype=tf.float32);
    ...
    tf.compat.v1.add_to_collection("KL_layers", kl_loss)

...
def show_predictions(model, testX):
    ...
    for s in list(session_groups.groups):
        ...
        for l in list(lap_groups.groups):
            ...
            print(pred)
            lap_prediction = pd.DataFrame(pred)
            lap_prediction.columns = ['prediction']
            lap_prediction['sessionUID'] = sessionUID
            print(f'lap shape {lap.shape}')
            print(f'prediction shape {pred.shape}')
            result = pd.concat([lap_prediction, lap],axis=1)
            lap_arr.append(result)

    predictions = pd.concat(lap_arr)
    plt.scatter(x=predictions['currentLapTime'], y=predictions['prediction'])
    plt.savefig('ANN_predictions.svg')
    predictions.to_csv('ANN_predictions.csv')
    plt.show()
    return predictions
...
def test_predictions(model, testX, testY):
    testX = testX.drop('sessionUID', axis=1)
    pred = pd.Series(model.predict(testX).ravel())
    print(pred)
    print(pred.shape)

...
def plot_residuals(df):
    ...
    plt.scatter(x=x, y=y, s=10, color='xkcd:lightish blue', alpha=0.8, zorder=1)
    # plt.fill_between(x, y - dy, y + dy, color='gray', alpha=0.2)
    # plt.title('Prediction Residuals for LSTM Model', fontsize=30, fontweight='bold')
    plt.xlabel('Current Lap Time (s)', fontsize=30, fontweight='bold')
    plt.ylabel('Residuals', fontsize=30, fontweight='bold')
    plt.xticks(fontsize=25)
    plt.yticks(fontsize=25)
    plt.text(80, -5 ,f'R2: {r_2}', fontsize=25, fontweight='bold')
    plt.axhline(y=0,  color ="green", linestyle ="--", linewidth=3,label='Target Value', zorder=2)
    # plt.savefig('LSTM_residuals.png', bbox_inches='tight',pad_inches = 0)
    plt.show()
...
def plot_residuals_by_sector(df):
    ...
    # fig.suptitle('Prediction Residuals Grouped by Sector for LSTM Model', fontsize=30, fontweight='bold')
    ...
    ax1.scatter(x = sector1_x , y = sector1_y,s=10, color='orange', alpha=0.8, zorder=1)
    ax2.scatter(x = sector2_x, y = sector2_y,s=10, color='darkcyan', alpha=0.8, zorder=1)
    ax3.scatter(x = sector3_x, y = sector3_y,s=10, color='indianred', alpha=0.8, zorder=1)
    ...
    ax1.text(10, 2, f'R2: {sector_1_r2}', fontsize=40, fontweight='bold')
    ax2.text(40, 2, f'R2: {sector_2_r2}', fontsize=40, fontweight='bold')
    ax3.text(70, 2, f'R2: {sector_3_r2}', fontsize=40, fontweight='bold')
    ...
    # plt.savefig('LSTM_residuals_by_sector.png', bbox_inches='tight',pad_inches = 0)
...
def plot_predictions(df):
    ...
    y = df['predictions']
    y2 = df['truth']
    plt.scatter(x=x, y=y, s=0.2, color='xkcd:lightish blue', alpha=0.8, zorder=1)
    plt.scatter(x=x, y=y2, s=0.2, color='orange', alpha=0.8, zorder=1)
    # plt.fill_between(x, y - dy, y + dy, color='gray', alpha=0.2)
    # plt.title('Prediction Residuals for LSTM Model', fontsize=30, fontweight='bold')
    plt.xlabel('Current Lap Time (s)', fontsize=30, fontweight='bold')
    plt.ylabel('Predictions (s)', fontsize=30, fontweight='bold')
    plt.xticks(fontsize=25)
    plt.yticks(fontsize=25)
    plt.savefig('LSTM_predictions.png', bbox_inches='tight',pad_inches = 0)
    plt.show()


if __name__ == '__main__':
    LSTM = pd.read_csv('LSTM_predictions_finalLapTime.csv')
    LSTM.drop([LSTM.columns[0]], axis=1, inplace=True)
    LSTM.dropna(inplace=True)

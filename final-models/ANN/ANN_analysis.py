...
def r_squared(df):
    finalLapTime = df['finalLapTime']
    pred = df['predictions']
    return r2_score(finalLapTime, pred)
...
def plot_residuals_by_sector(df):
    ...
    # fig.suptitle('Prediction Residuals Grouped by Sector for XGboost Model', fontsize=30, fontweight='bold')
    ...
    ax1.text(20, 0.1, f'R2: {sector_1_r2}', fontsize=40, fontweight='bold')
    ax2.text(50, 0.1, f'R2: {sector_2_r2}', fontsize=40, fontweight='bold')
    ax3.text(80, 0.1, f'R2: {sector_3_r2}', fontsize=40, fontweight='bold')
    ...
    plt.savefig('ANN_residuals_by_sector.png',bbox_inches='tight',pad_inches = 0)

    plt.show()

if __name__ == '__main__':
    xgb = pd.read_csv('ANN_predictions.csv')
    xgb.drop([xgb.columns[0]], axis=1, inplace=True)
    xgb['sector'] = xgb['sector'].apply(int)

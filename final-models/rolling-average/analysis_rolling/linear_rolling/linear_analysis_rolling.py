...
def plot_residuals(df):
    ...
    plt.savefig('linear_residuals_rolling.png', bbox_inches='tight',pad_inches = 0)
    plt.show()
...
def plot_residuals_by_sector(df):
    ...
    # fig.suptitle('Prediction Residuals Grouped by Sector for Linear Model', fontsize=30, fontweight='bold')
    ...
    ax1.scatter(x = sector1_x , y = sector1_y,s=0.5, color='orange', alpha=0.8, zorder=1)
    ax2.scatter(x = sector2_x, y = sector2_y,s=0.5, color='darkcyan', alpha=0.8, zorder=1)
    ax3.scatter(x = sector3_x, y = sector3_y,s=0.5, color='indianred', alpha=0.8, zorder=1)
    ...
    plt.savefig('linear_residuals_by_sector_rolling.png', bbox_inches='tight',pad_inches = 0)
...
def plot_linearity(df):
    ...
    actual_line = np.arange(actual.min(), actual.max())

    plt.figure(figsize=(14, 10))

    plt.plot(actual_line, actual_line, color='darkred', linestyle='--', linewidth=2)
    plt.xlabel('Actual Lap Time (S)', fontsize=25)
    plt.ylabel('Predicted Lap Time (S)', fontsize=25)

    plt.xticks(fontsize=30)
    plt.yticks(fontsize=30)
    plt.scatter(x = actual, y=predicted, alpha=0.7, s=3, color='darkcyan')
    plt.text(120, 122, 'Linear Baseline', rotation=36, fontsize=30, fontweight='bold')
    plt.savefig('linearity_rolling.png', bbox_inches='tight',pad_inches = 0)
    plt.show()

if __name__ == '__main__':
    linear = pd.read_csv('../../prediction_rolling_csv/Linear_rolling_predictions.csv')
